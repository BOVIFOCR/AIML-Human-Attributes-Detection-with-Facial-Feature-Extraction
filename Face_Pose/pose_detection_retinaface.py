...
detector_model = tf.saved_model.load(os.path.join(os.path.dirname(__file__), 'models/tf_retinaface_mbv2/'))
...
def draw_landmarks(frame, bb, points):
    ...
    cv2.circle(frame, (points[0], points[5]), 1, red, 2)# left eye
    cv2.circle(frame, (points[1], points[6]), 1, red, 2)# right eye
    cv2.circle(frame, (points[2], points[7]), 1, red, 2)# nose
    cv2.circle(frame, (points[3], points[8]), 1, red, 2)# mouth - left
    cv2.circle(frame, (points[4], points[9]), 1, red, 2)# mouth - right 
